/*
  # Agency White-labeling and OpenAI Key Management
  ...
  2. Security
    ...
    - Ensure only authorized agencies can manage their keys
*/
...
-- Update get_user_ghl_configuration function to include new columns
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'ghl_configurations' AND column_name = 'ghl_company_id'
  ) THEN
    ALTER TABLE ghl_configurations ADD COLUMN ghl_company_id text;
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'ghl_configurations' AND column_name = 'ghl_user_type'
  ) THEN
    ALTER TABLE ghl_configurations ADD COLUMN ghl_user_type text;
  END IF;
END $$;