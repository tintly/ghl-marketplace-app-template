/*
  ...
  This migration fixes issues with subscription management functions that were causing
  ambiguous column reference errors and permission issues.

  1. Changes
     - Fixed ambiguous column references in get_my_subscription_direct function
     - Fixed ambiguous column references in get_my_usage_with_limits function
     - Added force_change_subscription_plan function with minimal checks
     - Improved error handling in subscription-related functions
     - Ensured agency users always have agency plan and permissions

  2. Security
     - All functions are SECURITY DEFINER to ensure proper access control
     - Added checks to ensure users can only manage their own subscriptions
*/
