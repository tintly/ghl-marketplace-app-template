    ...
    return new Response(JSON.stringify({
      ...
      metadata: {
        ...
        fields: extractionFields.map((f)=>({
            ...
            fieldKey: getProperFieldKey(f), // This is the key used in the prompt's JSON output
            required: f.is_required,
            isStandard: isStandardField(f),
            overwritePolicy: f.overwrite_policy || 'always',
            description: f.description, // Include the field's description
            picklistOptions: f.picklist_options // Include picklist options
          }))
      }
    }), {
      status: 200,
...
function separateFieldTypes(extractionFields) {
...
function getProperFieldKey(field) {
  ...
  const sanitizedName = field.field_name.toLowerCase().replace(/[^a-z0-9\s]/g, '') // Remove special characters
...
async function getGHLConfiguration(supabase, locationId) {
  ...
  if (!data) {
    ...
    const { data: allConfigs, error: allError } = await supabase.from('ghl_configurations').select('id, ghl_account_id, business_name, is_active').limit(5);
...
async function getExtractionFields(supabase, configId) {
  ...
  if (fields.length === 0) {
    const { data: allFields, error: allError } = await supabase.from('data_extraction_fields').select('id, config_id, field_name, target_ghl_key').limit(10);
...
async function getContextualRules(supabase, configId) {
...
async function getStopTriggers(supabase, configId) {
...
function generatePromptWithSeparatedFields({ ghlConfig, extractionFields, contextualRules, stopTriggers, locationId }) {
  ...
  if (customFields.length > 0) {
    ...
    customFields.forEach((field)=>{
      ...
      if ([
        'SINGLE_OPTIONS',
        'MULTIPLE_OPTIONS'
      ].includes(field.field_type) && field.picklist_options?.length > 0) {
